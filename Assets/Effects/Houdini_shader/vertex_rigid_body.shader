...
Shader "sidefx/vertex_rigid_body_shader" {
	Properties {
		...
		_inputtime ("InputTime" , Range(0,0.99)) = 0
	}
	SubShader {
		...
		void vert(inout appdata_full v){
			...
			float timeInFrames = ((ceil(frac(_inputtime) * _numOfFrames))/_numOfFrames);//((ceil(frac(_Time.y * _speed) * _numOfFrames))/_numOfFrames);// + (1.0/_numOfFrames);
