    ...
    SubShader {
        Pass {
            ...
            uniform vec4 _NormalMap_ST;
                ...
                void main() {	            
                    glVertexWorld = _Object2World * gl_Vertex;      //UnityObjectToClipPos(v.vertex);

                    vec3 n = gl_Normal;//normalize((_Object2World * vec4(gl_Normal, 0.0)).xyz);
                    vec3 t = Tangent.xyz;//normalize((_Object2World * vec4(Tangent.xyz, 0.0)).xyz);
                ...
                void main() {
                    ...
                    * pow(max(0.0, dot(reflect(-lightDirectionNormal, surfaceNormal), viewDirectionNormal)), _SpecularExponent);                      
                    ...
                    vec3 hoge = vec3(1,1,1)*reflect(-lightDirectionNormal, surfaceNormal);
                    hoge = surfaceNormal;
                    gl_FragColor = vec4(hoge,1);//vec4(ambientLight + diffuseReflection + specularReflection, 1.0);
                }
            #endif
