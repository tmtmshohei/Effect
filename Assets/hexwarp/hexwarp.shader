	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i , fixed facing : VFACE) : SV_Target
			{
				...
				col.a = i.vcolor.a+1;
				//col = lerp(-1,1,col);
				col.rgb = lerp(col.rgb, float3(_threshold, _threshold, _threshold), i.normal);
				col.b = 0;
				fixed4 inside = tex2D(_inside, col.rg);
				fixed4 outside = tex2D(_outside, i.normal.rg);
				...
				//col *= i.vcolor*_col;
				if (i.normal.r < _threshold)discard;
				// apply fog
				col = float4(i.normal.r, i.normal.b, 0, 1);UNITY_APPLY_FOG(i.fogCoord, col);
