Shader "Noriben/GlareTrail"
{
	Properties
	{
		...
		_Gradation("Gradation", Range(0, 20)) = 1.0
		_HeadGradation ("HeadGradation", Range(0, 1)) = 1.0
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float grad = (1.- i.uv.x);
				float headgrad = smoothstep(0., _HeadGradation, i.uv.x);

				grad = pow(grad, _Gradation);
				
				color = saturate((color + centerColor) * grad * headgrad);
